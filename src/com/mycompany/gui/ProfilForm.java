...
public class ProfilForm extends Form{
    ...
    public ProfilForm(Resources res) {
     ...
     Toolbar tb = new Toolbar(true);
        ...
        User user=ServiceUser.getInstance().find(SessionManager.getId());
        
       TextField nomPers = new TextField(user.getNom(), "nom");
         TextField prenomPers = new TextField(user.getPrenom(), "prénom");
        TextField numTel = new TextField(user.getNum(), "numéro");
        
        TextField login = new TextField(user.getEmail(), "E-Mail", 20, TextField.EMAILADDR) ;
       Vector<String> vectorRole;
        vectorRole = new Vector();
        vectorRole.add("chauffeur");
    ...
    FontImage.setMaterialIcon(adresseIcon, FontImage.MATERIAL_LOCATION_ON, 3);
        ...
        loginButton.addActionListener(e -> {
            user.setIdUser(SessionManager.getId());
        user.setNom(nomPers.getText());
        user.setPrenom(prenomPers.getText());
        user.setNum(numTel.getText());
        user.setEmail(login.getText());
             System.out.println("persone-----------------"+user);
            if(ServiceUser.getInstance().modifierUser(user,user.getIdUser())){
                 System.out.println("persone-----------------"+user);
